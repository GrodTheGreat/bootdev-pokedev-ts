...
export class PokeAPI {
  ...
  async fetchLocation(locationName: string): Promise<LocationArea> {
    const entry = this.cache.get(locationName);
    if (entry) {
      return entry as LocationArea;
    }
  ...
  async fetchPokemon(pokemon: string): Promise<PokemonDetails> {
    const entry = this.cache.get(pokemon);
    if (entry) {
      return entry as PokemonDetails;
    }
    const response = await fetch(PokeAPI.baseURL + "/pokemon/" + pokemon);
...
type EncounterRateVersionDetail = {
  rate: number;
  version: EncounterRateVersion;
};

type EncounterRateVersion = {
  name: string;
  url: string;
};
